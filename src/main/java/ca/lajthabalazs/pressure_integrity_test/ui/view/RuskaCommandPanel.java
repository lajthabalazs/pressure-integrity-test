...
/**
 * Panel shown in the Ruska device window when connected. Provides a command chooser, Send button,
 * and a scrolling log of sent (grey italic) and received (black) messages.
 */
public class RuskaCommandPanel extends JPanel {
  ...
  public RuskaCommandPanel(SerialPortHandle handle) {
    ...
    for (Command c : RuskaReadCommands.ALL) {
    ...
    Command first = RuskaReadCommands.ALL.get(0);
              ...
              while (readerRunning && in != null) {
                try {
                  ...
                  while ((cr = line.indexOf("\r")) >= 0 || (cr = line.indexOf("\n")) >= 0) {
                    ...
                    if (!one.isEmpty()) {
                      String toShow = one;
                      appendToLog(formatMessageForDisplay(toShow), receivedStyle);
                    }
                  }
                } catch (IOException e) {
                  if (readerRunning) {
